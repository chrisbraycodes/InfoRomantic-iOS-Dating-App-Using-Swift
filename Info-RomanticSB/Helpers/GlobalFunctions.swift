...
func saveLikeToUser(userId: String) {
    ...
    if let currentUser = FUser.currentUser() {
        ...
        if !didLikeUserWith(userId: userId) {
            ...
            currentUser.updateCurrentUserInFireStore(withValues: [nLIKEDIDARRAY: currentUser.likedIdArray!]) { (error) in
...
func createRecentItems(chatRoomId: String, users: [FUser]) {
    ...
    FirebaseReference(.Recent).whereField(nCHATROOMID, isEqualTo: chatRoomId).getDocuments { (snapshot, error) in
...
func removeMemberWhoHasRecent(snapshot: QuerySnapshot, memberIds: [String]) -> [String] {
    ...
    for recentData in snapshot.documents {
        ...
        if let currentUserId = currentRecent[nSENDERID] {
